...
# LOSSQ_SECTIONED_EXCEL_LOSS_RUN_REPAIR_V1
def lossq_sectioned_excel_loss_run_repair_v1(file_path, parsed_claims=None, profile_data=None, direct_profile=None):
  """
  Universal sectioned Excel loss run repair.

  Handles Excel loss runs where profile data appears as label/value rows,
  exposure data appears in a section, and claims appear under a claims-detail
  header. Supports bilingual English/French labels without hardcoding a customer,
  carrier, policy, or uploaded filename.
  """
  import re
  ...
  ext = str(file_path or "").lower().rsplit(".", 1)[-1]
  if ext not in {"xlsx", "xlsm"}:
    return parsed_claims, profile_data, direct_profile
  ...
  try:
    ...
    print("LOSSQ_SECTIONED_EXCEL_LOSS_RUN_REPAIR_READ_ERROR_V1:", str(exc)[:200])
    return parsed_claims, profile_data, direct_profile
  ...
  first_lines = [" ".join(row).strip() for row in all_rows[:8] if " ".join(row).strip()]
  for line in first_lines:
    if re.search(r"(?i)\b(insurance|assurance|mutual|casualty|indemnity|underwriters|general insurance|assurances générales|assurances generales)\b", line):
      if not re.search(r"(?i)loss run|relevé|releve|sinistres|liability|responsabilit", line):
        carrier = clean(line)
        break

  for row in all_rows:
    label = clean_key(row[0] if len(row) > 0 else "")
    value = clean(row[1] if len(row) > 1 else "")

    if not label or not value:
      continue

    if not business_name and re.search(r"\b(assur|insured|named insured|nom assure|assuré|assure)\b", label, flags=re.I):
      business_name = value
      continue

    if not policy_number and re.search(r"\b(policy|police)\b", label, flags=re.I):
      policy_number = value
      continue

    if not province_raw and re.search(r"\b(province|state)\b", label, flags=re.I):
      province_raw = value
      province, province_name, regulator = province_code(value)
      continue

    if not currency and re.search(r"\b(currency|devise)\b", label, flags=re.I):
      currency = value.upper()
      continue

    if not line_of_business and re.search(r"\b(ibc|bac|line of business|ligne)\b", label, flags=re.I):
      line_of_business = value
      continue

    if not evaluation_date and re.search(r"\b(report date|date du rapport|valuation|evaluation|évaluation)\b", label, flags=re.I):
      evaluation_date = value
      continue

    if re.search(r"\b(period|période|periode)\b", label, flags=re.I):
  ...
  if not carrier:
    prepared_rows = [row for row in all_rows if re.search(r"(?i)\b(prepared by|préparé par|prepare par)\b", " ".join(row))]
    for row in prepared_rows:
      text = " ".join(row)
      text = re.sub(r"(?i).*?(prepared by|préparé par|prepare par)\s*[:#-]\s*", "", text).strip()
      text = re.sub(r"(?i)\s+[–-]\s+(commercial|personal|claims|underwriting|lignes commerciales).*$", "", text).strip()
      if text:
        carrier = clean(text)
        break

  # Exposure/policy rows.
  exposure_rows = []
  in_exposure = False
  exposure_headers = []
  ...
  for row in all_rows:
    joined = " ".join(row)
    if re.search(r"(?i)\b(exposure data|données d'exposition|donnees d exposition)\b", joined):
      in_exposure = True
      exposure_headers = []
      continue

    if in_exposure and re.search(r"(?i)\b(claims detail|détail des sinistres|detail des sinistres)\b", joined):
      in_exposure = False
      continue
    ...
    if not exposure_headers and re.search(r"(?i)\b(category|catégorie|categorie)\b", joined):
      exposure_headers = row
      continue

    if exposure_headers and row and clean(row[0]) and clean(row[0]).upper() != "TOTAL":
  ...
  for index, row in enumerate(all_rows):
    joined = " ".join(row)
    if re.search(r"(?i)\b(claim|sinistre)\b", joined) and re.search(r"(?i)\b(loss date|date sinistre|date de sinistre|claim #|n° sinistre|numero sinistre)\b", joined):
      header = row
      header_index = index
      break

  if header and header_index is not None:
    normalized_headers = [clean_key(item) for item in header]
    ...
    c_reserve = col(r"reserve", r"réserve", r"reserve")
    ...
    for row in all_rows[header_index + 1:]:
      ...
      joined = " ".join(row)
      if re.search(r"(?i)\b(notes|remarques|prepared by|préparé par|confidential|confidentiel)\b", joined):
        break

      claim_number = clean(row[c_claim]) if c_claim >= 0 and c_claim < len(row) else ""

      if not looks_like_claim_number(claim_number):
        continue

      paid = format_money(row[c_paid]) if c_paid >= 0 and c_paid < len(row) else "0"
      reserve = format_money(row[c_reserve]) if c_reserve >= 0 and c_reserve < len(row) else "0"
      incurred = format_money(row[c_incurred]) if c_incurred >= 0 and c_incurred < len(row) else ""

      if not incurred:
        incurred = str(money_float(paid) + money_float(reserve))

      coverage = clean(row[c_cov]) if c_cov >= 0 and c_cov < len(row) else line_of_business
      ...
      claim = {
        ...
        "loss_date": clean(row[c_loss]) if c_loss >= 0 and c_loss < len(row) else "",
        "reported_date": clean(row[c_report]) if c_report >= 0 and c_report < len(row) else "",
        "claimant": clean(row[c_claimant]) if c_claimant >= 0 and c_claimant < len(row) else "",
        "description": clean(row[c_desc]) if c_desc >= 0 and c_desc < len(row) else "",
        "status": status_value(row[c_status]) if c_status >= 0 and c_status < len(row) else "",
        "paid": paid,
        "paid_amount": paid,
        "reserve": reserve,
        "reserve_amount": reserve,
        ...
        "currency": currency,
        "state": province,
        "province": province_name or province,
        "source_parser": "sectioned_excel_loss_run",
      }

      claims.append(claim)

  existing_real_claims = [
  ...
  if claims and len(claims) >= len(existing_real_claims):
    parsed_claims = claims

  claim_count_by_coverage = {}
  incurred_by_coverage = {}

  for claim in claims:
    cov = clean(claim.get("coverage") or claim.get("line_of_business") or line_of_business) or line_of_business or "Policy"
    claim_count_by_coverage[cov] = claim_count_by_coverage.get(cov, 0) + 1
    incurred_by_coverage[cov] = incurred_by_coverage.get(cov, 0.0) + money_float(claim.get("total_incurred"))

  policy_rows = []

  if exposure_rows:
    for row in exposure_rows:
      coverage_name = clean(row[0])
      ...
      for value in row[1:]:
        ...
        if not limit and re.search(r"\d", clean_value):
          limit = clean_value

      # Premium is often the last numeric exposure value.
      numeric_values = [clean(value) for value in row[1:] if clean(value) and re.search(r"\d", clean(value))]
      if numeric_values:
        premium = numeric_values[-1]

      coverage_key = "D&O" if re.search(r"(?i)\b(d&o|administrateurs|directors)\b", coverage_name) else "E&O" if re.search(r"(?i)\b(e&o|professional|professionnelle|erreurs|omissions)\b", coverage_name) else coverage_name

      matching_incurred = 0.0
      matching_claims = 0

      for cov, total in incurred_by_coverage.items():
        if coverage_key.lower() in cov.lower() or cov.lower() in coverage_key.lower():
          matching_incurred += total
          matching_claims += claim_count_by_coverage.get(cov, 0)
      ...
      policy_rows.append({
        ...
        "total_incurred": matching_incurred,
      })

  if not policy_rows and policy_number:
    total_incurred = sum(money_float(claim.get("total_incurred")) for claim in claims)
  ...
  is_canada = bool(province or currency == "CAD" or re.search(r"(?i)\bcanada\b|\bcad\b|\bibc\b|\bbac\b|québec|quebec|ontario|manitoba", " ".join(first_lines)))

  def apply_profile(target):
    if not isinstance(target, dict):
      return
    ...
    if province:
      ...
      target["primary_state"] = province
      target["primaryState"] = province
      target["state_province"] = province
      target["stateProvince"] = province
      target["province_code"] = province
      target["provinceCode"] = province
      target["province"] = province_name or province
      target["province_name"] = province_name or province
      target["provinceName"] = province_name or province
    ...
    if province:
      market_context["state"] = province
      market_context["state_province"] = province
      market_context["stateProvince"] = province
      market_context["province"] = province
      market_context["province_code"] = province
      market_context["provinceCode"] = province
      market_context["province_name"] = province_name or province
  ...
  print("LOSSQ_SECTIONED_EXCEL_LOSS_RUN_REPAIR_V1:", {
    "business_name": business_name,
    "carrier": carrier,
    "policy_number": policy_number,
    "province": province,
...
async def save_uploaded_files(files, policy_number, db, current_user):
  ...
  # LOSSQ_UNIVERSAL_CLAIM_NUMBER_FILTER_V1
    ...
    # LOSSQ_SECTIONED_EXCEL_LOSS_RUN_REPAIR_CALL_V1
  parsed_claims, profile_data, direct_profile = lossq_sectioned_excel_loss_run_repair_v1(
    file_path,
    parsed_claims,
    profile_data,
    direct_profile,
