...
# LOSSQ_LABEL_BASED_PDF_POLICY_SCHEDULE_REBUILD_V3
def lossq_parse_label_based_pdf_loss_run_v3(file_path: str):
    """
    Strong universal parser for text-readable label-based PDFs.
    Rebuilds policy schedule from the POLICY SCHEDULE section only.
    This prevents Account Number / Customer Number values from becoming policy cards.
    """
    import re
    ...
    def infer_lob(policy_number, current_lob=""):
        current_lob = clean(current_lob)
        if current_lob and current_lob.upper() not in {"UNKNOWN", "N/A", "NA", "NONE", "NULL", "NOT SET"}:
            return current_lob
    ...
    try:
        ...
        print("LOSSQ_LABEL_PDF_V3_TEXT_EXTRACT_WARNING:", str(exc)[:500])
    ...
    # Rebuild policies only from POLICY SCHEDULE, stopping before claim/loss sections.
    schedule_text = raw_text
    schedule_match = re.search(
        r"POLICY SCHEDULE([\s\S]*?)(?:LOSS SUMMARY|CLAIM DETAIL|BROKER RECOMMENDATION|$)",
        raw_text,
        flags=re.I,
    )
    if schedule_match:
    ...
    policy_blocks = re.split(r"(?=Policy Number\s*:)", schedule_text, flags=re.I)

    policies = []
    seen_policy_numbers = set()

    def block_value(block, *labels):
        for label in labels:
            m = re.search(rf"^{re.escape(label)}\s*:\s*(.+)$", block, flags=re.I | re.M)
            if m:
                return clean(m.group(1))
        return ""

    for block in policy_blocks:
        policy_number = block_value(block, "Policy Number")

        if not policy_number:
            continue

        if is_account_identifier(policy_number):
            continue

        policy_key = compact(policy_number)
        if not policy_key or policy_key in seen_policy_numbers:
            continue

        seen_policy_numbers.add(policy_key)

        lob = block_value(block, "Line of Business", "Coverage", "Policy Type", "Policy Line")
        lob = infer_lob(policy_number, lob)

        pol_carrier = block_value(block, "Writing Carrier", "Carrier") or carrier
        pol_eff = block_value(block, "Effective Date", "Effective") or effective_date
        pol_exp = block_value(block, "Expiration Date", "Expiration") or expiration_date
        current_premium = money_text(block_value(block, "Current Premium"))
        expiring_premium = money_text(block_value(block, "Expiring Premium"))
        target_premium = money_text(block_value(block, "Target Renewal Premium"))
        exposure_basis = block_value(block, "Exposure Basis")
        exposure_value = money_text(block_value(block, "Exposure Value"))
        employee_count = money_text(block_value(block, "Employee Count"))
        vehicle_count = money_text(block_value(block, "Vehicle Count"))
        driver_count = money_text(block_value(block, "Driver Count"))
        property_tiv = money_text(block_value(block, "Property TIV"))
        ...
        policies.append({
            ...
            "carrier_name": pol_carrier,
            "writing_carrier": pol_carrier,
            "effective_date": pol_eff,
            "effective": pol_eff,
            "expiration_date": pol_exp,
            "expiration": pol_exp,
            "current_premium": current_premium,
            "premium": current_premium,
            "expiring_premium": expiring_premium,
    ...
    policy_lob_map = {compact(p.get("policy_number")): p.get("line_of_business") for p in policies}

    if policies:
        profile["policies"] = policies
        profile["policy_schedule"] = policies
        profile["policy_number"] = policies[0].get("policy_number", "")
        profile["main_policy_number"] = policies[0].get("policy_number", "")
    ...
    for policy in policies:
        try:
            current_total += float(money_text(policy.get("current_premium")) or 0)
            expiring_total += float(money_text(policy.get("expiring_premium")) or 0)
            target_total += float(money_text(policy.get("target_renewal_premium")) or 0)
        except Exception:
            pass
    ...
    # Parse claims from CLAIM DETAIL section only.
    claim_section_match = re.search(r"CLAIM DETAIL([\s\S]*?)(?:BROKER RECOMMENDATION|$)", raw_text, flags=re.I)
    claim_text = claim_section_match.group(1) if claim_section_match else raw_text
    claim_blocks = re.split(r"(?=Claim Number\s*:)", claim_text, flags=re.I)

    claims = []

    for block in claim_blocks:
        claim_number = block_value(block, "Claim Number")
        if not claim_number:
            continue

        claim_policy_number = block_value(block, "Policy Number")
        if is_account_identifier(claim_policy_number):
            claim_policy_number = ""

        lob = block_value(block, "Line of Business", "Coverage", "Policy Type")
        lob = policy_lob_map.get(compact(claim_policy_number)) or infer_lob(claim_policy_number, lob)

        paid = money_text(block_value(block, "Paid", "Paid Amount"))
        reserve = money_text(block_value(block, "Reserve"))
        incurred = money_text(block_value(block, "Total Incurred", "Incurred"))

        if not incurred:
            try:
                incurred_value = float(paid or 0) + float(reserve or 0)
                incurred = str(int(incurred_value)) if incurred_value.is_integer() else f"{incurred_value:.2f}"
            except Exception:
                incurred = ""
        ...
        claims.append({
            ...
            "policyNumber": claim_policy_number,
            "line_of_business": lob,
            "policy_type": lob,
            "policyType": lob,
            "coverage": lob,
            ...
            "line": lob,
            "lob": lob,
            "date_of_loss": block_value(block, "Date of Loss", "Loss Date"),
            "loss_date": block_value(block, "Date of Loss", "Loss Date"),
            "date_reported": block_value(block, "Date Reported", "Reported Date"),
            "date_closed": block_value(block, "Date Closed", "Closed Date"),
            "status": block_value(block, "Status") or ("Closed" if block_value(block, "Date Closed", "Closed Date") else "Open"),
            "paid": paid,
            "paid_amount": paid,
            "reserve": reserve,
            "total_incurred": incurred,
            "incurred": incurred,
            "cause_of_loss": block_value(block, "Cause of Loss", "Cause"),
            "description": block_value(block, "Description"),
            "litigation": block_value(block, "Litigation"),
        })
    ...
    profile["claims"] = claims
    profile["parsed_claims"] = claims
    profile["lossq_label_based_pdf_v3_detected"] = True
    profile["extraction_status"] = "passed"
    profile["extraction_score"] = 95
    profile["requires_review"] = False

    print("LOSSQ_LABEL_BASED_PDF_V3_PARSED:", {
        "claims": len(claims),
        "policies": [
            {"policy_number": p.get("policy_number"), "line_of_business": p.get("line_of_business")}
            for p in policies
        ],
        "business_name": profile.get("business_name"),
        "account_number": profile.get("account_number"),
...
def parse_file(file_path: str, filename: str):
  ...
  if lower_name.endswith(".pdf"):
      # LOSSQ_LABEL_BASED_PDF_V3_PRIORITY_BEFORE_OLD_PDF_PARSE
      try:
          label_pdf_v3_claims, label_pdf_v3_profile = lossq_parse_label_based_pdf_loss_run_v3(file_path)
          if label_pdf_v3_claims:
              return label_pdf_v3_claims, label_pdf_v3_profile
      except Exception as label_pdf_v3_exc:
          print("LOSSQ_LABEL_BASED_PDF_V3_PARSE_ERROR:", str(label_pdf_v3_exc)[:500])
