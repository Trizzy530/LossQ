...
# LOSSQ_PDF_FINAL_PROFILE_DISPLAY_ALIAS_SYNC_V1
def lossq_pdf_final_profile_display_alias_sync_v1(file_path, profile_data=None, direct_profile=None):
  """
  Final PDF profile-display alias sync.
  ...
  def read_pdf_text():
    try:
      ...
      print("LOSSQ_PDF_FINAL_PROFILE_DISPLAY_ALIAS_SYNC_READ_ERROR_V1:", str(exc)[:200])
      return ""
  ...
  if province_match:
    province_value = clean(province_match.group(1))
  ...
  policy_number = clean(profile_data.get("policy_number") or profile_data.get("main_policy_number") or direct_profile.get("policy_number") or direct_profile.get("main_policy_number"))
  ...
  if policy_match:
    policy_number = clean(policy_match.group(1))

  lob = clean(profile_data.get("line_of_business") or profile_data.get("policy_type") or profile_data.get("coverage") or direct_profile.get("line_of_business") or direct_profile.get("policy_type") or direct_profile.get("coverage"))
  ...
  if period_match:
    policy_period = clean(period_match.group(1))

  effective_date = clean(profile_data.get("effective_date") or profile_data.get("effectiveDate") or profile_data.get("effective") or direct_profile.get("effective_date") or direct_profile.get("effectiveDate") or direct_profile.get("effective"))
  expiration_date = clean(profile_data.get("expiration_date") or profile_data.get("expirationDate") or profile_data.get("expiration") or direct_profile.get("expiration_date") or direct_profile.get("expirationDate") or direct_profile.get("expiration"))
  ...
  if dates_match:
    effective_date = clean(dates_match.group(1))
    expiration_date = clean(dates_match.group(2))

  report_date = clean(profile_data.get("evaluation_date") or profile_data.get("evaluationDate") or profile_data.get("valuation_date") or profile_data.get("valuationDate") or profile_data.get("report_date") or profile_data.get("reportDate"))
  ...
  is_canada = bool(province_code or currency == "CAD" or re.search(r"(?i)\bCanada\b|\bCAD\b|\bIBC\s+\d+\b|\bWSIB\b", normalized))
  ...
  def apply(target):
    ...
    if report_date:
      ...
      target["evaluationDate"] = report_date
      target["valuation_date"] = report_date
      target["valuationDate"] = report_date
      target["report_date"] = report_date
      target["reportDate"] = report_date
    ...
    if province_code:
      ...
      target["state_name"] = province_name or province_code
      target["stateName"] = province_name or province_code
    ...
    if province_code:
      market_context["state"] = province_code
      market_context["primary_state"] = province_code
      market_context["primaryState"] = province_code
      market_context["state_province"] = province_code
      ...
      market_context["province_name"] = province_name or province_code
      market_context["provinceName"] = province_name or province_code
    ...
    for exposure_key in ("exposure_inputs", "exposureInputs", "manual_exposure_inputs", "manualExposureInputs"):
      exposure = target.get(exposure_key)
      if isinstance(exposure, dict):
        if province_code:
          exposure["state"] = province_code
          exposure["State"] = province_code
          exposure["primary_state"] = province_code
          exposure["primaryState"] = province_code
          exposure["State / Province"] = province_code
          exposure["state_province"] = province_code
          exposure["stateProvince"] = province_code
          exposure["province"] = province_name or province_code
          exposure["province_code"] = province_code
          exposure["provinceCode"] = province_code
        if currency:
          exposure["currency"] = currency
          exposure["Currency"] = currency

    policies = target.get("policy_schedule") or target.get("policySchedule") or target.get("policies")
    if not isinstance(policies, list):
      policies = []

    has_policy = any(isinstance(row, dict) and clean(row.get("policy_number") or row.get("policyNumber")) for row in policies)

    if policy_number and not has_policy:
      row = {
        "policy_number": policy_number,
        "policyNumber": policy_number,
        "line_of_business": lob,
      ...
      for row in policies:
        ...
        if clean(row.get("policy_number") or row.get("policyNumber")) == policy_number:
          if effective_date:
          ...
          if expiration_date:
            ...
            row["expirationDate"] = expiration_date
            row["expiration"] = expiration_date
          if province_code:
            row["state"] = province_code
            row["stateProvince"] = province_code
            row["province"] = province_name or province_code
          if currency:
            row["currency"] = currency
          if lob:
            row["line_of_business"] = row.get("line_of_business") or lob
            row["lineOfBusiness"] = row.get("lineOfBusiness") or lob
            row["policy_type"] = row.get("policy_type") or lob
            row["policyType"] = row.get("policyType") or lob
            row["coverage"] = row.get("coverage") or lob
          if carrier:
            row["carrier"] = row.get("carrier") or carrier
            row["carrier_name"] = row.get("carrier_name") or carrier
            row["carrierName"] = row.get("carrierName") or carrier
            row["writing_carrier"] = row.get("writing_carrier") or carrier
            row["writingCarrier"] = row.get("writingCarrier") or carrier

    target["policies"] = policies
    target["policy_schedule"] = policies
    target["policySchedule"] = policies

  apply(profile_data)
  apply(direct_profile)

  print("LOSSQ_PDF_FINAL_PROFILE_DISPLAY_ALIAS_SYNC_V1:", {
    "province_code": province_code,
    "regulator": regulator,
    "effective_date": effective_date,
    "expiration_date": expiration_date,
...
def lossq_final_fix_claim_detail_v3(normalized_claim, raw_claim):
  ...
  cause_of_loss = (
    lossq_final_first_v3(normalized_claim, "cause_of_loss", "cause", "loss_cause", "cause of loss", "loss cause", "cause description")
    or lossq_final_first_v3(raw_claim, "cause_of_loss", "cause", "loss_cause", "cause of loss", "loss cause", "cause description")
  )

  description = (
    lossq_final_first_v3(normalized_claim, "description", "loss_description", "claim_description", "narrative", "claim notes", "loss notes")
    or lossq_final_first_v3(raw_claim, "description", "loss_description", "claim_description", "narrative", "claim notes", "loss notes")
  )

  if not cause_of_loss and description:
  ...
  if cause_of_loss:
    normalized_claim["cause_of_loss"] = clean_cause_of_loss(cause_of_loss)

  if description and not normalized_claim.get("description"):
    normalized_claim["description"] = description
  ...
  normalized_claim.pop("loss_cause", None)
  normalized_claim.pop("cause", None)
  normalized_claim.pop("source_file", None)
...
def lossq_universal_tabular_upload_canonical_save_overlay_v1(
  ...
  def real_claim_number(value):
    raw = clean(value)
    key = compact_key(raw)
    if not raw or key in fake_claim_heading_keys:
      return False
    if re.search(r"(?i)\b(report basis|closed claims|open claims|loss ratio|loss summary|policy schedule|premium worksheet|underwriting notes)\b", raw):
      return False
    # Real carrier claim numbers normally contain digits. Keep numeric-only claim
    # numbers too; the amount/date checks below will protect section headings.
    return bool(re.search(r"\d", raw))

  def is_real_claim_row(claim):
    if not isinstance(claim, dict):
      return False
    claim_number = clean(claim.get("claim_number") or claim.get("Claim Number") or claim.get("claimNumber"))
    if not real_claim_number(claim_number):
      return False
    amount_total = (
      money_float(claim.get("paid_amount") or claim.get("paid"))
      + money_float(claim.get("reserve_amount") or claim.get("reserve"))
      + money_float(claim.get("total_incurred") or claim.get("incurred") or claim.get("total"))
    )
    date_value = clean(
      claim.get("date_of_loss")
      or claim.get("loss_date")
      or claim.get("date_reported")
      or claim.get("reported_date")
    )
    policy_or_line = clean(claim.get("policy_number") or claim.get("policy") or claim.get("line_of_business") or claim.get("claim_type"))
    return amount_total > 0 or bool(date_value and policy_or_line)

  def clean_claim_rows(rows):
    cleaned = []
    seen = set()
    for claim in rows or []:
      if not is_real_claim_row(claim):
        continue
      copy_claim = dict(claim)
      claim_number = clean(copy_claim.get("claim_number") or copy_claim.get("Claim Number") or copy_claim.get("claimNumber"))
      policy_number = clean(copy_claim.get("policy_number") or copy_claim.get("Policy Number") or copy_claim.get("policyNumber"))
      key = f"{claim_number.upper()}|{policy_number.upper()}"
      if key in seen:
        continue
      seen.add(key)
      copy_claim["claim_number"] = claim_number
      cleaned.append(copy_claim)
    return cleaned

  def policy_is_upload_fallback(value):
    return bool(re.match(r"(?i)^UPLOAD-\d+(?:-\d+)?$", clean(value)))

  def safe_business_value(value):
    value = clean(value).strip(" :-|/")
  ...
  if str(file_path or "").lower().endswith(".csv"):
    ...
    try:
      ...
      csv_profile_label_map = {
        ...
        "lineofbusiness": "primary_line_of_business",
        "coverage": "primary_line_of_business",
      ...
      for row in rows[:80]:
        ...
        table_header_signals = {
          "policynumber", "policyno", "policy", "lineofbusiness", "coverage",
          "policytype", "effectivedate", "effdate", "expirationdate", "expdate",
          "currentpremium", "annualpremium", "claimnumber", "claimno",
          "dateofloss", "lossdate", "datereported", "paid", "paidlosses",
          "reserve", "reserves", "totalincurred",
        }
        if len(row_keys.intersection(table_header_signals)) >= 3:
          continue
        for idx, cell in enumerate(row):
          field = csv_profile_label_map.get(row_key(cell))
          if not field:
            continue
      ...
      if not clean(parsed_profile.get("carrier_name") or parsed_profile.get("writing_carrier")):
        for row in rows[:12]:
          row_values = [clean(cell) for cell in row if clean(cell)]
          if len(row_values) != 1:
            continue
          candidate = row_values[0]
          if re.search(r"(?i)\b(insurance|mutual|underwriters|casualty|indemnity|specialty|risk)\b", candidate):
            parsed_profile["carrier_name"] = candidate.title() if candidate.isupper() else candidate
            parsed_profile["writing_carrier"] = parsed_profile["carrier_name"]
            break

      semantic_claim_rows = []
      semantic_claim_header_idx = None
      semantic_claim_headers = []
      for idx, row in enumerate(rows):
        row_keys = {row_key(cell) for cell in row if clean(cell)}
        if (
          any(option in row_keys for option in {"claimnumber", "claimno", "claimid", "claim"})
          and any(option in row_keys for option in {"dateofloss", "lossdate", "datereported", "paidlosses", "paid", "reserves", "reserve", "totalincurred", "incurred"})
        ):
          semantic_claim_header_idx = idx
          semantic_claim_headers = row
          break

      if semantic_claim_header_idx is not None:
        for row in rows[semantic_claim_header_idx + 1:]:
          row_text = " ".join(clean(cell).lower() for cell in row if clean(cell))
          if not any(clean(cell) for cell in row):
            break
          if row_text.startswith("totals") or row_text.startswith("total,") or "loss ratio" in row_text or "status key" in row_text:
            break
          ...
          if not real_claim_number(claim_number):
            continue
          policy_number = first(data, "Policy Number", "Policy No", "Policy") or parsed_profile.get("policy_number", "")
          paid = first_money(data, "Paid Losses", "Paid Loss", "Paid", "Paid Amount", "Total Paid")
          reserve = first_money(data, "Reserves", "Reserve", "Reserve Amount", "Outstanding Reserve")
          incurred = first_money(data, "Total Incurred", "Incurred", "Gross Incurred", "Net Incurred")
          if money_float(incurred) <= 0 and (money_float(paid) > 0 or money_float(reserve) > 0):
            incurred = money_float(paid) + money_float(reserve)
          status = first(data, "Status", "Claim Status")
          if not status:
            status = "Open" if money_float(reserve) > 0 else "Closed"

          claim_row = {
            "claim_number": claim_number,
            "policy_number": policy_number,
            "line_of_business": first(data, "Line of Business", "Coverage / Line", "Coverage", "Policy Type", "LOB"),
            "claim_type": first(data, "Line of Business", "Coverage / Line", "Coverage", "Policy Type", "LOB"),
            "claimant": first(data, "Claimant", "Claimant / Party", "Claimant Party", "Claimant Name", "Injured Worker", "Injured Party", "Employee Name", "Plaintiff", "Customer Name", "Third Party", "Third Party Name"),
            "cause_of_loss": first(data, "Cause of Loss", "Loss Cause", "Cause", "Cause Description"),
            "description": first(data, "Description", "Loss Description", "Claim Description", "Narrative", "Cause of Loss", "Loss Cause", "Cause"),
            "status": status,
            "claim_status": status,
            "date_of_loss": first(data, "Date of Loss", "Loss Date"),
          ...
          if is_real_claim_row(claim_row):
            semantic_claim_rows.append(claim_row)

      semantic_policy_rows = []
      if clean(parsed_profile.get("policy_number") or parsed_profile.get("policy_type")):
        policy_line = clean(parsed_profile.get("policy_type") or parsed_profile.get("primary_line_of_business"))
        semantic_policy_rows.append({
          "policy_number": clean(parsed_profile.get("policy_number")),
      ...
      for idx, row in enumerate(rows):
        ...
        if section_seen and row_matches:
          ...
          break
        if any(word in row_text for word in ["exposure", "policy information", "policy schedule", "premium worksheet", "premium schedule", "policy worksheet"]):
          section_seen = True
      ...
      if exposure_idx is not None:
        for row in rows[exposure_idx + 1:]:
          ...
          strong_exposure_rows.append({
            ...
            "carrier_name": first(data, "Carrier", "Writing Carrier") or parsed_profile.get("carrier_name", ""),
            "writing_carrier": first(data, "Writing Carrier", "Carrier") or parsed_profile.get("writing_carrier") or parsed_profile.get("carrier_name", ""),
            "state": first(data, "State", "Jurisdiction/State", "Jurisdiction"),
            "effective_date": effective_date,
            "expiration_date": expiration_date,
    ...
    try:
      ...
      if not clean(parsed_profile.get("carrier_name") or parsed_profile.get("writing_carrier")):
        for value in workbook_text[:40]:
          candidate = clean(value)
          candidate_low = candidate.lower()
          if not candidate or "loss run" in candidate_low or "report" in candidate_low:
            continue
          if "the hartford" in candidate_low:
            parsed_profile["carrier_name"] = "The Hartford"
            parsed_profile["writing_carrier"] = "The Hartford"
            break
          if re.search(r"(?i)\b(insurance|mutual|underwriters|specialty|casualty|indemnity|financial services group)\b", candidate):
            parsed_profile["carrier_name"] = candidate.title() if candidate.isupper() else candidate
            parsed_profile["writing_carrier"] = parsed_profile["carrier_name"]
            break

      if not clean(parsed_profile.get("state")):
        state_codes = {
        ...
        for value in workbook_text[:80]:
          for match in re.finditer(r"\b([A-Z]{2})\b", clean(value).upper()):
            if match.group(1) in state_codes:
              parsed_profile["state"] = match.group(1)
              break
          if parsed_profile.get("state"):
            break
      ...
      for worksheet in workbook.worksheets:
        ...
        for idx, row in enumerate(rows):
          ...
          claim_header = (
            ...
            and (
              any(option in row_keys for option in {"policynumber", "policyno", "policy"})
              or clean(parsed_profile.get("policy_number"))
            )
          )
          ...
          if policy_header:
            ...
            for data_row in rows[idx + 1:]:
              ...
              workbook_policies.append({
                ...
                "state": first_cell(data, "State", "Jurisdiction/State", "Jurisdiction"),
          ...
          if claim_header:
            ...
            for data_row in rows[idx + 1:]:
              ...
              policy_number = first_cell(data, "Policy Number", "Policy No", "Policy") or parsed_profile.get("policy_number", "")
              if not claim_number:
                continue
              ...
              claim_row = {
                ...
                "claimant": first_cell(data, "Claimant", "Claimant / Party", "Claimant Party", "Claimant Name", "Injured Worker", "Injured Party", "Employee Name", "Plaintiff", "Customer Name", "Third Party", "Third Party Name"),
                ...
                "jurisdiction_state": first_cell(data, "Jurisdiction/State", "Jurisdiction", "State", "Venue State", "Loss State"),
                "venue_state": first_cell(data, "Jurisdiction/State", "Jurisdiction", "State", "Venue State", "Loss State"),
                "adjuster": first_cell(data, "Adjuster", "Adjuster/Examiner", "Examiner", "Claim Adjuster", "Claim Examiner", "File Handler"),
                "examiner": first_cell(data, "Examiner", "Adjuster/Examiner", "Adjuster", "Claim Examiner", "File Handler"),
                "cause_of_loss": first_cell(data, "Cause of Loss", "Loss Cause", "Cause", "Cause Description"),
                "paid_amount": paid,
                "paid": paid,
                "reserve_amount": reserve,
                "reserve": reserve,
                ...
                "attorney_assigned": first_cell(data, "Attorney Assigned", "Attorney", "Attorney / Suit Status", "Counsel", "Represented"),
              ...
              if is_real_claim_row(claim_row):
                workbook_claims.append(claim_row)

      if workbook_policies:
        context["exposure_rows"] = workbook_policies
      elif clean(parsed_profile.get("policy_number")):
        policy_line = clean(parsed_profile.get("policy_type") or parsed_profile.get("primary_line_of_business") or parsed_profile.get("line_of_business"))
        context["exposure_rows"] = [{
          "policy_number": clean(parsed_profile.get("policy_number")),
          "policy_type": policy_line,
          "line_of_business": policy_line,
          "coverage": policy_line,
          ...
          "effective_date": parsed_profile.get("effective_date", ""),
          "expiration_date": parsed_profile.get("expiration_date", ""),
          "current_premium": money_value(parsed_profile.get("current_premium")),
        }]
      if workbook_claims:
  ...
  section_claims = clean_claim_rows(section_claims)
  parsed_claims = clean_claim_rows(parsed_claims)
  ...
  if policies:
    if len(policies) == 1:
      only_policy = clean(policies[0].get("policy_number"))
      only_line = clean(policies[0].get("line_of_business") or policies[0].get("policy_type"))
      for claim in parsed_claims:
        if not isinstance(claim, dict):
          continue
        if only_policy and (not clean(claim.get("policy_number")) or policy_is_upload_fallback(claim.get("policy_number"))):
          claim["policy_number"] = only_policy
          claim["policy"] = only_policy
        if only_line and not clean(claim.get("line_of_business") or claim.get("claim_type")):
          claim["line_of_business"] = only_line
          claim["claim_type"] = only_line
    elif len(policies) > 1:
      policy_by_line = {}
      for policy in policies:
        policy_number = clean(policy.get("policy_number"))
        line_key = compact_key(policy.get("line_of_business") or policy.get("policy_type") or policy.get("coverage"))
        if policy_number and line_key:
          policy_by_line.setdefault(line_key, policy_number)
      for claim in parsed_claims:
        if not isinstance(claim, dict):
          continue
        existing_policy = clean(claim.get("policy_number"))
        if existing_policy and not policy_is_upload_fallback(existing_policy):
          continue
        line_key = compact_key(claim.get("line_of_business") or claim.get("claim_type") or claim.get("coverage"))
        if line_key in policy_by_line:
          claim["policy_number"] = policy_by_line[line_key]
          claim["policy"] = policy_by_line[line_key]
    ...
    for policy in policies:
      policy_number = clean(policy.get("policy_number")).upper()
      if not policy_number:
        continue
      policy["claim_count"] = claim_counts.get(policy_number, 0)
      policy["total_incurred"] = claim_totals.get(policy_number, 0.0)
    ...
    if not parsed_profile.get("state"):
      for policy in policies:
        policy_state = clean(policy.get("state")).upper()
        if re.fullmatch(r"[A-Z]{2}", policy_state or ""):
          parsed_profile["state"] = policy_state
          break
...
def lossq_canada_canonical_upload_overlay_v1(
  ...
  if ext.endswith((".csv", ".txt")):
    ...
    try:
      ...
      def pdf_section(start_pattern, end_patterns):
        ...
        for pattern in end_patterns:
          stop = re.search(pattern, tail, flags=re.I)
          if stop:
            stops.append(stop.start())
        return tail[:min(stops)] if stops else tail

      def pdf_section_lines(start_pattern, end_patterns):
        return [clean(row) for row in pdf_section(start_pattern, end_patterns).splitlines() if clean(row)]

      def first_policy_date_format():
        fmt = key(pdf_next_label_value("Date Format", "Format de date"))
        if fmt:
          context["source_date_format"] = pdf_next_label_value("Date Format", "Format de date")
        return fmt

      context["business_name"] = context.get("business_name") or pdf_next_label_value("Named Insured", "Insured", "Assuré")
      context["account_number"] = context.get("account_number") or pdf_next_label_value("Account Number", "Customer Number", "Client Number")
      context["customer_number"] = context.get("customer_number") or context.get("account_number")
      context["producing_agency"] = context.get("producing_agency") or pdf_next_label_value("Producing Agency", "Brokerage", "Broker", "Courtier")
      context["agency_name"] = context.get("agency_name") or context.get("producing_agency")
      context["producer"] = context.get("producer") or pdf_next_label_value("Primary Contact", "Producer", "Contact principal")
      writing_carrier_label = pdf_next_label_value("Writing Carrier", "Insurance Carrier", "Carrier", "Insurer", "Assureur")
      if good(writing_carrier_label) and not re.search(r"(?i)sample data only|not an actual insurance record|deductible|policy number|line of business", writing_carrier_label):
        context["carrier_name"] = context.get("carrier_name") or writing_carrier_label
        context["writing_carrier"] = context.get("writing_carrier") or writing_carrier_label

      market_country_label = pdf_next_label_value("Market / Country", "Market/Country", "Country / Market", "Pays")
      if market_country_label:
        province_hits = []
        for province_name_key, province_code_value in province_names.items():
          display_name = province_display(province_code_value)
          if re.search(rf"\b{re.escape(display_name)}\b", market_country_label, flags=re.I):
            province_hits.append(province_code_value)
        for province_code_value in province_codes:
          if re.search(rf"\b{re.escape(province_code_value)}\b", market_country_label):
            province_hits.append(province_code_value)
        province_hits = list(dict.fromkeys(province_hits))
        if province_hits:
          context["province_code"] = context.get("province_code") or province_hits[0]
          context["province"] = context.get("province") or province_display(province_hits[0])
          context["market_region_codes"] = province_hits
          context["market_region"] = " / ".join(province_hits)

      valuation_label = pdf_next_label_value("Valuation Date", "Evaluation Date", "Run Date", "Report Date", "Date d'évaluation")
      if valuation_label:
        context["evaluation_date"] = ca_date(valuation_label)
        context["valuation_date"] = ca_date(valuation_label)

      loss_run_period_label = pdf_next_label_value("Loss Run Period", "Policy Period", "Coverage Period", "Période")
      if loss_run_period_label:
        parse_policy_period(loss_run_period_label)
      first_policy_date_format()

      exposure_inputs = context.get("exposure_inputs") if isinstance(context.get("exposure_inputs"), dict) else {}
      exposure_lines = pdf_section_lines(
        r"\bEXPOSURE\s+SCHEDULE\b",
        [r"\bEXPOSURE\s+MARKUP\s+NOTES\b", r"\bCLAIM\s+DETAIL\b", r"\bLOSS\s+SUMMARY\b"],
      )
      exposure_header_keys = {"exposurefield", "value", "basisunderwritinguse", "parserfieldname"}
      exposure_idx = 0
      while exposure_idx + 3 < len(exposure_lines):
        field_label = exposure_lines[exposure_idx]
        value_label = exposure_lines[exposure_idx + 1]
        parser_field_raw = clean(exposure_lines[exposure_idx + 3])
        parser_field = re.sub(r"[^a-zA-Z0-9_]+", "_", parser_field_raw).strip("_").lower()
        if key(field_label) in exposure_header_keys or not parser_field:
          exposure_idx += 1
          continue
        parsed_value = amount(value_label) if re.search(r"(?i)\bCAD\b|\$|\d[\d,]{3,}", value_label) else clean(value_label)
        if parsed_value not in ("", None, [], {}):
          explicit_exposure_fields.add(parser_field)
          context[parser_field] = parsed_value
          exposure_inputs[parser_field] = parsed_value
          exposure_inputs[field_label] = parsed_value
        exposure_idx += 4
      if exposure_inputs:
        context["exposure_inputs"] = exposure_inputs
      ...
      structured_policies = []
      policy_section_lines = pdf_section_lines(
        r"\bPOLICY\s+SCHEDULE\b",
        [r"\bEXPOSURE\s+SCHEDULE\b", r"\bCLAIM\s+DETAIL\b", r"\bLOSS\s+SUMMARY\b"],
      )
      policy_number_re = re.compile(r"^[A-Z0-9]+(?:-[A-Z0-9]+){2,}$")
      date_re = re.compile(r"^\d{4}[/-]\d{1,2}[/-]\d{1,2}$|^\d{1,2}[/-]\d{1,2}[/-]\d{2,4}$")
      money_re = re.compile(r"(?i)^(?:CAD|CDN|CA\$|C\$|\$)?\s*[0-9][0-9,]*(?:\.\d{2})?(?:\s*(?:CAD|CDN))?$")
      policy_header_words = {
        "policynumber", "lineofbusiness", "effective", "expiration",
        "limitattachment", "deductiblesir", "premium",
      }
      repaired_policy_lines = []
      idx = 0
      while idx < len(policy_section_lines):
        value = policy_section_lines[idx]
        if (
          policy_number_re.match(value)
          and idx + 4 < len(policy_section_lines)
          and re.fullmatch(r"[A-Z0-9]{1,4}", policy_section_lines[idx + 1] or "")
          and not date_re.match(policy_section_lines[idx + 1])
          and not date_re.match(policy_section_lines[idx + 2])
          and date_re.match(policy_section_lines[idx + 3])
          and date_re.match(policy_section_lines[idx + 4])
        ):
          repaired_policy_lines.append(f"{value}{policy_section_lines[idx + 1]}")
          idx += 2
          continue
      ...
      idx = 0
      while idx < len(repaired_policy_lines):
        policy_number = repaired_policy_lines[idx]
        if not policy_number_re.match(policy_number):
          idx += 1
          continue
        cursor = idx + 1
        line_parts = []
        while cursor < len(repaired_policy_lines) and not date_re.match(repaired_policy_lines[cursor]):
          if key(repaired_policy_lines[cursor]) not in policy_header_words:
            line_parts.append(repaired_policy_lines[cursor])
          cursor += 1
        if cursor + 1 >= len(repaired_policy_lines):
          idx += 1
          continue
        effective = repaired_policy_lines[cursor]
        expiration = repaired_policy_lines[cursor + 1]
        if not (date_re.match(effective) and date_re.match(expiration)):
          idx += 1
          continue
        cursor += 2
        detail_parts = []
        while cursor < len(repaired_policy_lines) and not policy_number_re.match(repaired_policy_lines[cursor]):
          if key(repaired_policy_lines[cursor]) not in policy_header_words:
            detail_parts.append(repaired_policy_lines[cursor])
          cursor += 1
        money_parts = [piece for piece in detail_parts if money_re.match(piece)]
        structured_policies.append({
          "policy_number": policy_number,
          "policy": policy_number,
          "line_of_business": line(" ".join(line_parts)),
          "policy_type": line(" ".join(line_parts)),
          "coverage": line(" ".join(line_parts)),
          ...
          "effective_date": ca_date(effective),
          "expiration_date": ca_date(expiration),
          "current_premium": amount(money_parts[-1]) if money_parts else context.get("current_premium"),
          "deductible": amount(money_parts[-2]) if len(money_parts) > 1 else "",
          "limits": " ".join(piece for piece in detail_parts if piece not in money_parts),
      ...
      if structured_policies:
        ...
        context["policy_number"] = context.get("policy_number") or structured_policies[0].get("policy_number")
        context["effective_date"] = structured_policies[0].get("effective_date") or context.get("effective_date")
        context["expiration_date"] = structured_policies[0].get("expiration_date") or context.get("expiration_date")
      ...
      structured_claims = []
      claim_section_lines = pdf_section_lines(
        r"\bCLAIM\s+DETAIL\b",
        [r"\bLOSS\s+SUMMARY\b", r"\bUNDERWRITING\s+MARKUP\b", r"\bSUBMISSION\s+READINESS\b"],
      )
      claim_start_re = re.compile(r"^[A-Z0-9]+(?:-[A-Z0-9]+)*-\d{5,6}$")
      claim_blocks = []
      current_claim_block = []
      for claim_line in claim_section_lines:
        if key(claim_line) in {
          "claimnumber", "policynumber", "lob", "claimant", "jurisdiction",
          "adjusterexaminer", "lossdate", "reported", "closed", "status",
          "paid", "reserve", "incurred", "claimnotes",
        }:
          continue
        if claim_start_re.match(claim_line):
          if current_claim_block:
            claim_blocks.append(current_claim_block)
          current_claim_block = [claim_line]
        elif current_claim_block:
          current_claim_block.append(claim_line)
      if current_claim_block:
        claim_blocks.append(current_claim_block)
      ...
      def claim_policy_from_block(block):
        if len(block) < 2:
          return "", 1
        raw_policy = block[1]
        consumed = 1
        if raw_policy.upper() not in policy_lookup and len(block) > 2 and re.fullmatch(r"[A-Z0-9]{1,4}", block[2] or ""):
          candidate = f"{raw_policy}{block[2]}"
          if candidate.upper() in policy_lookup:
            raw_policy = candidate
            consumed = 2
        if raw_policy.upper() not in policy_lookup:
          for known_policy in policy_lookup:
            if known_policy.startswith(raw_policy.upper()) or known_policy.endswith("-" + raw_policy.upper()):
              raw_policy = clean(policy_lookup[known_policy].get("policy_number"))
              break
        return raw_policy, consumed

      for block in claim_blocks:
        claim_number = block[0]
        policy_number, consumed_policy_parts = claim_policy_from_block(block)
        if not policy_number:
          continue
        dates = [(idx, value) for idx, value in enumerate(block) if date_re.match(value)]
        money_items = [(idx, value) for idx, value in enumerate(block) if money_re.match(value) and re.search(r"(?i)\bCAD\b|CA\$|C\$|\$", value)]
        if len(dates) < 2 or len(money_items) < 3:
          continue
        first_date_idx = dates[0][0]
        pre_date_parts = block[1 + consumed_policy_parts:first_date_idx]
        policy_line = line((policy_lookup.get(policy_number.upper()) or {}).get("line_of_business") or "")
        raw_lob_parts = []
        claim_party_parts = pre_date_parts[:]
        if policy_line:
          policy_line_parts = policy_line.split()
          if [key(part) for part in claim_party_parts[:len(policy_line_parts)]] == [key(part) for part in policy_line_parts]:
            raw_lob_parts = claim_party_parts[:len(policy_line_parts)]
            claim_party_parts = claim_party_parts[len(policy_line_parts):]
        if not raw_lob_parts and claim_party_parts:
          first_lob_key = key(claim_party_parts[0])
          if first_lob_key in lob_aliases:
            raw_lob_parts = [claim_party_parts[0]]
            claim_party_parts = claim_party_parts[1:]
            if first_lob_key == "equipment" and claim_party_parts and key(claim_party_parts[0]) == "breakdown":
              raw_lob_parts.append(claim_party_parts[0])
              claim_party_parts = claim_party_parts[1:]
        claim_line_name = policy_line or line(" ".join(raw_lob_parts))

        jurisdiction = context.get("province_code") or context.get("province")
        claimant_parts = claim_party_parts
        adjuster_parts = []
        for part_idx, part in enumerate(claim_party_parts):
          if province_code(part):
            jurisdiction = province_code(part)
            claimant_parts = claim_party_parts[:part_idx]
            adjuster_parts = claim_party_parts[part_idx + 1:]
            break

        status_idx = None
        claim_status = ""
        for part_idx, part in enumerate(block):
          if key(part) in {"open", "closed", "ouvert", "ouverte", "ferme", "fermee", "clos"}:
            status_idx = part_idx
            claim_status = status(part)
            break
        if status_idx is None:
          claim_status = "Closed" if len(dates) > 2 else "Open"

        paid = amount(money_items[-3][1])
        reserve = amount(money_items[-2][1])
        incurred = amount(money_items[-1][1])
        if (incurred == "" or money(incurred) <= 0) and (money(paid) > 0 or money(reserve) > 0):
          incurred = money(paid) + money(reserve)
        ...
        structured_claims.append({
          ...
          "policy_number": policy_number,
          "policy": policy_number,
          "line_of_business": claim_line_name,
          "claim_type": claim_line_name,
          "coverage": claim_line_name,
          "claimant": clean(" ".join(claimant_parts)),
          "jurisdiction_state": jurisdiction,
          "venue_state": jurisdiction,
          "adjuster": clean(" ".join(adjuster_parts)),
          "examiner": clean(" ".join(adjuster_parts)),
          "date_of_loss": ca_date(dates[0][1]),
          "loss_date": ca_date(dates[0][1]),
          "date_reported": ca_date(dates[1][1]),
          "reported_date": ca_date(dates[1][1]),
          "date_closed": ca_date(dates[2][1]) if len(dates) > 2 and claim_status == "Closed" else "",
          "closed_date": ca_date(dates[2][1]) if len(dates) > 2 and claim_status == "Closed" else "",
          "status": claim_status,
          "claim_status": claim_status,
          "paid_amount": paid,
          "paid": paid,
          ...
          "total_incurred": incurred,
          "incurred": incurred,
          "description": clean(" ".join(block[money_items[-1][0] + 1:])),
          "loss_description": clean(" ".join(block[money_items[-1][0] + 1:])),
          "currency": "CAD",
          "market_currency": "CAD",
          "country": "Canada",
        })
      ...
      for idx, row in enumerate(lines):
        ...
        while pos + 4 < len(lines):
          ...
          summary_line = line(re.sub(r"\(?\b\d{4,6}\b\)?", "", coverage_name).strip(" /-") or coverage_name)
          ...
          coverage_key_part = matched_codes[0] if matched_codes else key(summary_line).upper()[:20]
          coverage_policy_number = f"{main_policy}-{coverage_key_part}" if main_policy and coverage_key_part else main_policy
          coverage_summary_rows.append({
            "policy_number": coverage_policy_number,
            "policy": coverage_policy_number,
            "source_policy_number": main_policy,
            "main_policy_number": main_policy,
            "base_policy_number": main_policy,
      ...
      if main_policy and coverage_summary_rows:
        ...
        for row in coverage_summary_rows:
          row_codes = [code for code in re.split(r"\s*/\s*", clean(row.get("ibc_code") or row.get("class_code"))) if code]
          coverage_policy_map.append({
            "policy_number": row.get("policy_number"),
            "line_key": key(row.get("line_of_business")),
            "codes": row_codes,
          })
        for claim in claims:
          claim_codes = [code for code in re.split(r"\s*/\s*", clean(claim.get("ibc_code") or claim.get("class_code"))) if code]
          claim_line_key = key(claim.get("line_of_business"))
          for row in coverage_policy_map:
            code_match = bool(set(claim_codes) & set(row.get("codes") or []))
            line_match = bool(claim_line_key and row.get("line_key") and (claim_line_key in row.get("line_key") or row.get("line_key") in claim_line_key))
            if code_match or line_match:
  ...
  for target in (parsed_profile, profile_data, direct_profile):
    ...
    if business_name:
      ...
      target["account_name"] = business_name
    if context.get("policy_number"):
      target["policy_number"] = context.get("policy_number")
      target["main_policy"] = context.get("policy_number")
      target["main_policy_number"] = context.get("policy_number")
    if carrier:
      target["carrier_name"] = carrier
    ...
    if (
      ext.endswith(".pdf")
      and "property_tiv" not in explicit_exposure_fields
      and clean(target.get("property_tiv"))
      and clean(target.get("property_tiv")) == clean(target.get("job_site_count"))
    ):
      target["property_tiv"] = ""
      target["tiv"] = ""
    ...
    if cleaned_policies:
      ...
      if target.get("effective_date"):
        ...
        exposure_inputs["Effective Date"] = target.get("effective_date")
      if target.get("expiration_date"):
        exposure_inputs["expiration_date"] = target.get("expiration_date")
        exposure_inputs["Expiration Date"] = target.get("expiration_date")
      ...
      if target.get("class_code"):
        exposure_inputs["classCode"] = target.get("class_code")
        exposure_inputs["class_codes"] = target.get("class_code")
        exposure_inputs["Class Code(s)"] = target.get("class_code")
        exposure_inputs["Class Codes"] = target.get("class_code")
      if target.get("ibc_code"):
...
def lossq_us_pdf_policy_claim_table_repair_v1(file_path, parsed_claims=None, parsed_profile=None):
  ...
  def labeled_value(label):
    ...
    m = re.search(rf"(?im)^\s*{re.escape(label)}\s+(.+?)\s*$", raw_text)
    if m:
      return clean(m.group(1))
  ...
  if not insured_name or insured_name.lower() in {"business name", "insured", "named insured"}:
    insured_match = re.search(
      r"(?is)\bNamed\s+Insured\s*/\s*Business\s+Name\s*:?\s*(.+?)(?=\n\s*(?:Report\s+Valuation\s+Date|Account\s+No\.?|Account\s+Number|Customer\s+Number|Writing\s+Carrier|Carrier\b|Policy\s+Term)\b|$)",
      raw_text,
    )
    if insured_match:
  ...
  if not evaluation_date:
    valuation_match = re.search(r"(?im)\bValuation\s+Date\s*:\s*([0-9/-]+)", raw_text)
    evaluation_date = valuation_match.group(1) if valuation_match else ""
  ...
  def add_policy(policy):
    ...
    if key in policy_seen:
      return
    policy_seen.add(key)
    line = clean(policy.get("line_of_business") or policy.get("policy_type") or policy.get("coverage"))
    policies.append({
      "policy_number": policy_number,
      "line_of_business": line,
      "policy_type": line,
      "coverage": line,
      "carrier_name": clean(policy.get("carrier_name") or policy.get("carrier") or carrier),
      "writing_carrier": clean(policy.get("writing_carrier") or policy.get("carrier") or carrier),
      "effective_date": us_date(policy.get("effective_date") or effective_date),
      "expiration_date": us_date(policy.get("expiration_date") or expiration_date),
      "current_premium": money_number(policy.get("current_premium") or policy.get("premium")),
      "exposure_basis": clean(policy.get("exposure_basis")),
      "exposure_value": money_number(policy.get("exposure_value")),
      "claim_count": 0,
      "total_incurred": 0,
    })

  def section_between(start_pattern, end_patterns):
    start = re.search(start_pattern, raw_text, flags=re.I)
    if not start:
      return ""
  ...
  for block in re.split(r"(?i)\bPolicy\s+Block\s+\d+\b", policy_block_section):
    policy_number_match = re.search(r"(?im)^\s*Policy\s+Number\s*:\s*([A-Z0-9._/-]+)", block)
    if not policy_number_match:
      continue
    line_match = re.search(r"(?im)^\s*(?:Line\s+of\s+Business|Coverage|Policy\s+Type)\s*:\s*(.+?)\s*$", block)
    add_policy({
      "policy_number": policy_number_match.group(1),
      "line_of_business": line_match.group(1) if line_match else "",
      "effective_date": (re.search(r"(?im)^\s*Effective\s+Date\s*:\s*([0-9/-]+)", block) or [None, effective_date])[1],
      "expiration_date": (re.search(r"(?im)^\s*Expiration\s+Date\s*:\s*([0-9/-]+)", block) or [None, expiration_date])[1],
      "current_premium": (re.search(r"(?im)^\s*Current\s+Premium\s*:\s*(\$?\s*[0-9][0-9,]*(?:\.\d{2})?)", block) or [None, ""])[1],
      "exposure_basis": (re.search(r"(?im)^\s*Exposure\s+Basis\s*:\s*(.+?)\s*$", block) or [None, ""])[1],
      "exposure_value": (re.search(r"(?im)^\s*Exposure\s+Value\s*:\s*(.+?)\s*$", block) or [None, ""])[1],
    })

  table_policy_section = policy_block_section
  for line in [clean(part) for part in table_policy_section.splitlines()]:
    if not re.match(r"^[A-Z0-9]+-[A-Z0-9-]*\d[A-Z0-9-]*\s+", line):
      continue
    dates = list(re.finditer(r"\b\d{1,4}[/-]\d{1,2}[/-]\d{1,4}\b", line))
    if len(dates) < 2:
      continue
    policy_number = line.split()[0]
    before_dates = clean(line[len(policy_number):dates[0].start()])
    after_dates = clean(line[dates[1].end():])
    line_name = before_dates
    if carrier and carrier in before_dates:
      line_name = clean(before_dates.split(carrier, 1)[0])
    premium_match = re.search(r"\$?\s*[0-9][0-9,]*(?:\.\d{2})?", after_dates)
    add_policy({
      "policy_number": policy_number,
      "line_of_business": line_name,
      "effective_date": dates[0].group(0),
      "expiration_date": dates[1].group(0),
      "current_premium": premium_match.group(0) if premium_match else "",
    })

  vertical_policy_lines = [clean(part) for part in table_policy_section.splitlines() if clean(part)]
  policy_number_re = re.compile(r"^[A-Z0-9]+(?:-[A-Z0-9]+){2,}$")
  ...
  while idx < len(vertical_policy_lines):
    ...
    if not policy_number_re.match(value):
      idx += 1
      continue
    if idx + 5 >= len(vertical_policy_lines):
      break
    line_name = vertical_policy_lines[idx + 1]
    policy_carrier = vertical_policy_lines[idx + 2]
    eff = vertical_policy_lines[idx + 3]
    exp = vertical_policy_lines[idx + 4]
    prem = vertical_policy_lines[idx + 5]
    if not (date_re.match(eff) and date_re.match(exp) and money_re.match(prem)):
      idx += 1
      continue
    add_policy({
      "policy_number": value,
      ...
      "expiration_date": exp,
      "current_premium": prem,
      "exposure_basis": vertical_policy_lines[idx + 6] if idx + 6 < len(vertical_policy_lines) else "",
      "exposure_value": vertical_policy_lines[idx + 7] if idx + 7 < len(vertical_policy_lines) else "",
    })
    idx += 8
  ...
  while idx < len(vertical_policy_lines):
    ...
    if not policy_number_re.match(value):
      idx += 1
      continue
    if idx + 4 >= len(vertical_policy_lines):
      break
    line_name = vertical_policy_lines[idx + 1]
    eff = vertical_policy_lines[idx + 2]
    exp = vertical_policy_lines[idx + 3]
    prem = vertical_policy_lines[idx + 4]
    if not (date_re.match(eff) and date_re.match(exp) and money_re.match(prem)):
      idx += 1
      continue
    add_policy({
  ...
  def add_claim(claim):
    claim_number = clean(claim.get("claim_number"))
    policy_number = clean(claim.get("policy_number"))
    if not claim_number or not re.search(r"\d", claim_number):
      return
    key = f"{claim_number}|{policy_number}".upper()
    if key in claim_seen:
      return
    claim_seen.add(key)
    paid = money_number(claim.get("paid") or claim.get("paid_amount"))
    reserve = money_number(claim.get("reserve") or claim.get("reserve_amount"))
    incurred = money_number(claim.get("total_incurred") or claim.get("incurred") or claim.get("total"))
    if not incurred:
      try:
        incurred = str(float(paid or 0) + float(reserve or 0))
      except Exception:
        incurred = ""
    line = clean(claim.get("line_of_business") or claim.get("claim_type") or claim.get("coverage"))
    description = clean(claim.get("description"))
    cause = clean(claim.get("cause_of_loss") or claim.get("loss_cause") or claim.get("cause")) or description
    claims.append({
      "claim_number": claim_number,
      "policy_number": policy_number,
      "line_of_business": line,
      "claim_type": line,
      "claimant": clean(claim.get("claimant") or claim.get("claimant_name") or claim.get("claimant_party")),
      "jurisdiction_state": clean(claim.get("jurisdiction_state") or claim.get("jurisdiction") or claim.get("state") or claim.get("venue_state") or claim.get("loss_state")),
      "venue_state": clean(claim.get("venue_state") or claim.get("jurisdiction_state") or claim.get("jurisdiction") or claim.get("state") or claim.get("loss_state")),
      "adjuster": clean(claim.get("adjuster") or claim.get("examiner") or claim.get("adjuster_examiner") or claim.get("file_handler")),
      "examiner": clean(claim.get("examiner") or claim.get("adjuster") or claim.get("adjuster_examiner") or claim.get("file_handler")),
      "date_of_loss": us_date(claim.get("date_of_loss") or claim.get("loss_date")),
      "loss_date": us_date(claim.get("date_of_loss") or claim.get("loss_date")),
      "date_reported": us_date(claim.get("date_reported") or claim.get("reported_date")),
      "date_closed": us_date(claim.get("date_closed") or claim.get("closed_date")),
      "status": clean(claim.get("status") or claim.get("claim_status")),
      "claim_status": clean(claim.get("status") or claim.get("claim_status")),
      "cause_of_loss": cause,
      "paid_amount": paid,
      "paid": paid,
      "reserve_amount": reserve,
      ...
      "description": description,
      "litigation": clean(claim.get("litigation")),
      "attorney_assigned": clean(claim.get("attorney_assigned") or claim.get("attorney") or claim.get("counsel")),
      "flag": clean(claim.get("flag")),
      "business_name": insured_name,
  ...
  for block in re.split(r"(?i)\bClaim\s+Block\s+\d+\b", claim_block_section):
    claim_number_match = (
      re.search(r"(?im)^\s*Claim\s+Number\s*:\s*([A-Z0-9._/-]+)", block)
      or re.search(r"(?im)^\s*Claim\s+Number\s*$\s*^\s*([A-Z0-9._/-]+)", block)
    )
    if not claim_number_match:
      continue
    policy_match = (
      re.search(r"(?im)^\s*Policy\s+Number\s*:\s*([A-Z0-9._/-]+)", block)
      or re.search(r"(?im)\bPolicy\s+Number\s*$\s*^\s*([A-Z0-9._/-]+)", block)
      or re.search(r"(?im)^\s*Claim\s+Number\s*$\s*^\s*[A-Z0-9._/-]+\s+Policy\s+Number\s*$\s*^\s*([A-Z0-9._/-]+)", block)
    )
    def block_label_value(*labels):
      for label in labels:
        match = (
          re.search(rf"(?im)^\s*{re.escape(label)}\s*:\s*(.+?)\s*$", block)
          or re.search(rf"(?im)^\s*{re.escape(label)}\s*$\s*^\s*(.+?)\s*$", block)
        )
        if match:
    ...
    add_claim({
      ...
      "line_of_business": block_label_value("Line of Business", "Coverage", "Policy Type"),
      "date_of_loss": block_label_value("Date of Loss", "Loss Date"),
      "date_reported": block_label_value("Date Reported", "Reported Date"),
      "date_closed": block_label_value("Date Closed", "Closed Date"),
      "status": block_label_value("Status", "Claim Status"),
      "claimant": block_label_value("Claimant", "Claimant / Party", "Claimant Party", "Injured Party", "Plaintiff"),
      "jurisdiction_state": block_label_value("Jurisdiction/State", "Jurisdiction", "State", "Venue State", "Loss State"),
      "adjuster": block_label_value("Adjuster", "Adjuster/Examiner", "Examiner", "Claim Adjuster", "Claim Examiner", "File Handler"),
      "paid": block_label_value("Paid", "Paid Amount"),
      "reserve": block_label_value("Reserve", "Reserve Amount"),
      "total_incurred": block_label_value("Total Incurred", "Gross Incurred"),
      "cause_of_loss": block_label_value("Cause of Loss", "Loss Cause", "Cause"),
      "description": block_label_value("Description", "Loss Description", "Claim Description", "Narrative"),
      "litigation": block_label_value("Litigation", "Litigated"),
      "attorney_assigned": block_label_value("Attorney Assigned", "Attorney", "Counsel", "Represented"),
    })

  for line in [clean(part) for part in claim_block_section.splitlines()]:
    if not re.match(r"^[A-Z0-9]+-[A-Z0-9-]*\d[A-Z0-9-]*\s+[A-Z0-9]+-[A-Z0-9-]*\d[A-Z0-9-]*\s+", line):
      continue
    m = re.match(
      r"^(\S+)\s+(\S+)\s+(.+?)\s+(Open|Closed|Reopened|Pending|Denied)\s+(\d{1,4}[/-]\d{1,2}[/-]\d{1,4})\s+(\d{1,4}[/-]\d{1,2}[/-]\d{1,4})(?:\s+(\d{1,4}[/-]\d{1,2}[/-]\d{1,4}))?\s+(\$?\s*[0-9][0-9,]*(?:\.\d{2})?)\s+(\$?\s*[0-9][0-9,]*(?:\.\d{2})?)\s+(\$?\s*[0-9][0-9,]*(?:\.\d{2})?)\s*(.*)$",
      line,
      flags=re.I,
    )
    if not m:
    ...
    add_claim({
      ...
      "policy_number": m.group(2),
      "line_of_business": m.group(3),
      "status": m.group(4),
      "date_of_loss": m.group(5),
      "date_reported": m.group(6),
      "date_closed": m.group(7) or "",
      "paid": m.group(8),
      "reserve": m.group(9),
      "total_incurred": m.group(10),
      "description": m.group(11),
      "cause_of_loss": m.group(11),
    })

  vertical_claim_lines = [clean(part) for part in claim_block_section.splitlines() if clean(part)]
  claim_number_re = re.compile(r"^[A-Z0-9]+(?:-[A-Z0-9]+)+$")
  header_words = {
    "claim number", "policy number", "line", "status", "loss date", "reported",
    "closed", "paid", "reserve", "total", "description", "litigation", "flag",
  }
  idx = 0
  while idx < len(vertical_claim_lines):
    value = vertical_claim_lines[idx]
    if value.lower() in header_words or not claim_number_re.match(value):
      idx += 1
      continue
    if idx + 8 >= len(vertical_claim_lines):
      break
    policy_number = vertical_claim_lines[idx + 1]
    if not policy_number_re.match(policy_number):
      idx += 1
      continue
    line_name = vertical_claim_lines[idx + 2]
    status = vertical_claim_lines[idx + 3]
    loss_date = vertical_claim_lines[idx + 4]
    reported = vertical_claim_lines[idx + 5]
    if status.lower() not in {"open", "closed", "reopened", "pending", "denied"} or not date_re.match(loss_date) or not date_re.match(reported):
      idx += 1
      continue
    cursor = idx + 6
    closed_date = ""
    if cursor < len(vertical_claim_lines) and date_re.match(vertical_claim_lines[cursor]):
      closed_date = vertical_claim_lines[cursor]
      cursor += 1
    if cursor + 2 >= len(vertical_claim_lines):
      break
    paid = vertical_claim_lines[cursor]
    reserve = vertical_claim_lines[cursor + 1]
    total = vertical_claim_lines[cursor + 2]
    if not (money_re.match(paid) and money_re.match(reserve) and money_re.match(total)):
      idx += 1
      continue
    cursor += 3
    description_parts = []
    litigation = ""
    flag = ""
    while cursor < len(vertical_claim_lines):
      piece = vertical_claim_lines[cursor]
      if piece.lower() in {"yes", "no"}:
        litigation = piece
        cursor += 1
        if cursor < len(vertical_claim_lines) and not claim_number_re.match(vertical_claim_lines[cursor]) and vertical_claim_lines[cursor].lower() not in header_words:
          flag = vertical_claim_lines[cursor]
          cursor += 1
        break
      if claim_number_re.match(piece) and cursor + 1 < len(vertical_claim_lines) and policy_number_re.match(vertical_claim_lines[cursor + 1]):
        break
      if piece.lower() not in header_words:
    ...
    add_claim({
      ...
      "cause_of_loss": " ".join(description_parts),
      "litigation": litigation,
  ...
  if parsed_claims:
    parsed_profile["claim_count"] = len(parsed_claims)
    parsed_profile["total_claims"] = len(parsed_claims)
    parsed_profile["open_claims"] = sum(1 for claim in parsed_claims if isinstance(claim, dict) and "closed" not in clean(claim.get("status") or claim.get("claim_status")).lower())
    parsed_profile["closed_claims"] = sum(1 for claim in parsed_claims if isinstance(claim, dict) and "closed" in clean(claim.get("status") or claim.get("claim_status")).lower())
    total_paid = 0.0
    total_reserve = 0.0
    total_incurred = 0.0
    for claim in parsed_claims:
      if not isinstance(claim, dict):
        continue
      try:
        total_paid += float(money_number(claim.get("paid_amount") or claim.get("paid")) or 0)
        total_reserve += float(money_number(claim.get("reserve_amount") or claim.get("reserve")) or 0)
        total_incurred += float(money_number(claim.get("total_incurred") or claim.get("incurred") or claim.get("total")) or 0)
      except Exception:
        pass
    parsed_profile["total_paid"] = int(total_paid) if total_paid.is_integer() else total_paid
    parsed_profile["total_reserve"] = int(total_reserve) if total_reserve.is_integer() else total_reserve
    parsed_profile["total_incurred"] = int(total_incurred) if total_incurred.is_integer() else total_incurred
    parsed_profile["claims"] = parsed_claims
    parsed_profile["parsed_claims"] = parsed_claims
...
# LOSSQ_US_CHUBB_PROFESSIONAL_LINES_DETAIL_ROW_REPAIR_V2
def lossq_us_chubb_professional_lines_detail_row_repair_v2(file_path, parsed_claims, parsed_profile):
  """
  US-only Chubb Professional Lines PDF repair.

  Fixes:
  - Full policy number CBB-MPL-9934-2022 instead of MPL-9934-2022
  - Claims count 6 instead of 0
  - Total incurred from detail claim rows
  - Policy schedule populated from detail claims

  Canada guard is intentionally strict. This does not run on CAD/Canada/Province/
  Desjardins/Aviva/IBC/AMF files.
  """
  try:
    import re
    ...
    if not (
      "CHUBB INSURANCE" in upper_text and
      "PROFESSIONAL LINES" in upper_text and
      "CLAIMS HISTORY DETAIL" in upper_text and
      "POLICY NUMBER:" in upper_text and
      "COVERAGE LINES:" in upper_text
    ):
      return parsed_claims, parsed_profile
    ...
    def first_match(pattern, default=""):
      ...
      return re.sub(r"\s+", " ", clean(match.group(1))).strip(" :-|")

    full_policy_number = first_match(r"Policy\s+Number:\s*([A-Z0-9][A-Z0-9\-]{5,})")

    if not full_policy_number:
      return parsed_claims, parsed_profile

    named_insured = first_match(r"Named\s+Insured:\s*(.*?)\s+Policy\s+Number:")
    policy_type = first_match(r"Policy\s+Type:\s*(.*?)\s*(?:FEIN:|Coverage\s+Lines:|Policy\s+Period:)", "Management & Professional Liability")
    coverage_raw = first_match(r"Coverage\s+Lines:\s*(.*?)\s*(?:Policy\s+Period:|Run\s+Date:|Agent:)", "E&O / D&O / Cyber")
    ...
    # Parse each detail claim row directly from the US PDF text.
    row_pattern = re.compile(
      ...
      r"(E&O|D&O|Cyber)\s+"
      r"(.*?)\s+"
      r"(Negligent Advice\s*-\s*M&A|Breach of Fiduciary Duty|Data Breach Response|Failure to Deliver\s*-\s*Contract|Securities Disclosure|BEC\s*-\s*Wire Fraud)\s+"
      r"(Open|Closed)\s+"
      r"\$?([0-9,]+)\s+"
      r"\$?([0-9,]+)\s+"
      r"\$?([0-9,]+)",
    ...
    for match in row_pattern.finditer(raw_text):
      ...
      claimant = re.sub(r"\s+", " ", clean(match.group(5)))
      allegation = re.sub(r"\s+", " ", clean(match.group(6)))
      status = clean(match.group(7)).title()
      paid = money_number(match.group(8))
      reserve = money_number(match.group(9))
      total = paid + reserve
      ...
      raw_claims.append({
        ...
        "loss_description": allegation,
        "cause_of_loss": allegation,
    ...
    # Replace if current parser missed detail rows or saved summary/blank rows.
    existing_by_number = {}
    ...
    for claim in parsed_claims:
      if isinstance(claim, dict):
        ...
        if number:
          existing_by_number[number] = claim

    repaired_claims = []

    for raw_claim in raw_claims:
      number = raw_claim["claim_number"]
      ...
      repaired_claims.append(merged)

    # If parser had extra unrelated real rows, keep them only when not summary rows.
    for claim in parsed_claims:
      if not isinstance(claim, dict):
        continue
      ...
      if number and number not in {item["claim_number"] for item in raw_claims} and number.startswith("CBB-"):
        claim["policy_number"] = full_policy_number
        claim["policyNumber"] = full_policy_number
    ...
    parsed_profile.update({
      ...
      "run_date": run_date,
      "agency_name": agent or parsed_profile.get("agency_name"),
      "producing_agency": agent or parsed_profile.get("producing_agency"),
      "producer": agent or parsed_profile.get("producer"),
      "annual_premium": annual_premium or parsed_profile.get("annual_premium"),
      "current_premium": annual_premium or parsed_profile.get("current_premium"),
      "country_market": "United States",
      "country": "United States",
      ...
      "total_reserve": reserve_total,
      "total_incurred": incurred_total,
      "lossq_us_chubb_professional_lines_detail_row_repair_v2": True,
    })

    print("LOSSQ_US_CHUBB_PROFESSIONAL_LINES_DETAIL_ROW_REPAIR_V2:", {
      "policy_number": full_policy_number,
      "claims": claim_count,
      "total_incurred": incurred_total,
    })
    ...
    print("LOSSQ_US_CHUBB_PROFESSIONAL_LINES_DETAIL_ROW_REPAIR_V2_ERROR:", str(exc)[:500])
    return parsed_claims, parsed_profile
...
async def save_uploaded_files(files, policy_number, db, current_user):
  ...
  for file in files:
    ...
    # LOSSQ_US_CHUBB_PROFESSIONAL_LINES_DETAIL_ROW_REPAIR_CALL_V2
