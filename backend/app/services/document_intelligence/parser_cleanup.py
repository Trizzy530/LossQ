...
def _extract_after_label(raw_text: str, labels: List[str]) -> str:
    ...
    for i, line in enumerate(lines):
        line_clean = _normalize_spaces(line)
        for label in labels:
            if line_clean.lower() == label.lower():
                if i + 1 < len(lines):
                    next_line = _normalize_spaces(lines[i + 1])
                    if next_line:
                        return next_line
...
def _extract_business_name(raw_text: str) -> str:
    value = _extract_after_label(
        raw_text,
        ["Named Insured", "Insured Name", "Business Name", "Account Name", "Applicant"],
    )
    if value:
        return value
...
def _extract_carrier_name(raw_text: str) -> str:
    value = _extract_after_label(
        raw_text,
        ["Writing Carrier", "Carrier Name", "Insurer Name", "Insurer", "Insurance Company", "Company"],
    )
    if value:
        return value
...
def _extract_policy_rows(raw_text: str) -> List[Dict[str, Any]]:
    ...
    for line in raw_text.splitlines():
        ...
        for match in POLICY_RE.finditer(clean_line):
            policy_number = _normalize_policy_number(match.group(0))
            if not policy_number or policy_number in seen:
                continue

            lob = _line_of_business_from_text(clean_line) or _line_of_business_from_text(policy_number) or "Unknown"
...
def _claim_policy_from_claim_number(claim_number: str, policies: List[Dict[str, Any]]) -> str:
    ...
    for policy in policies:
        ...
        if upper.startswith("WC-") and "-WC-" in policy_number:
            return policy_number
        if upper.startswith(("CG-", "MT-")) and "-CG-" in policy_number:
            return policy_number
    return ""
...
def _is_fake_header_claim(claim: Dict[str, Any], policies: List[Dict[str, Any]]) -> bool:
    ...
    if claim_upper in {value.upper() for value in KNOWN_FAKE_CLAIM_VALUES}:
        return True
...
def _extract_claim_rows_from_raw_text(raw_text: str, policies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    claim_number_re = re.compile(r"\b(?:AL|GL|WC|CG)-\d{2}-\d{3,8}\b", re.IGNORECASE)
    ...
    for line in raw_text.splitlines():
        ...
        matches = list(claim_number_re.finditer(clean_line))
        if not matches:
            continue

        for claim_match in matches:
            ...
            if claim_number in seen or claim_number in KNOWN_FAKE_CLAIM_VALUES:
                continue
...
def cleanup_loss_run_extraction(
    ...
    cleaned_existing_claims = _clean_existing_claims(existing_claims, policies)
    cleaned_existing_claims = filter_fake_claims(cleaned_existing_claims)

    raw_claims = _extract_claim_rows_from_raw_text(raw_text, policies) if raw_text else []
    raw_claims = filter_fake_claims(raw_claims)

    final_claims = cleaned_existing_claims
    if raw_claims and len(raw_claims) > len(final_claims):
        final_claims = raw_claims

    parsed["raw_parsed_claims"] = raw_claims
    parsed["claims"] = final_claims
    parsed["parsed_claims"] = final_claims
    parsed["claim_count"] = len(final_claims)
    parsed["total_incurred"] = _claim_total(final_claims) if final_claims else 0.0
