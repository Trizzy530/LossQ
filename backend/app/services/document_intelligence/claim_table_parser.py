...
def detect_litigation(text: str) -> bool:
    lower = clean_text(text).lower()
    if any(phrase in lower for phrase in ["no litigation", "closed no litigation", "litigation no", "no attorney", "no attorney involvement"]):
        return False
    return any(term in lower for term in ["litigation", "litigated", "attorney", "suit filed", "lawsuit", "represented"])
...
def extract_policy_number(text: str, policies: list[dict]) -> str:
    ...
    sorted_policies = sorted(
        [normalize_policy_number(policy.get("policy_number")) for policy in policies if policy.get("policy_number")],
        key=len,
        reverse=True,
    )
