...
# LOSSQ_SECTIONED_EXCEL_POLICY_SUMMARY_RESULT_WRAPPER_V1
# Post-process Policy Summary / Claims Detail Excel workbooks after the core parser runs.
# This avoids stale period-as-policy-number and 5-year summary rows without touching
# saved files, upload history, profile deletion, or frontend cache.
try:
    _lossq_sectioned_excel_original_repair_v1
except NameError:
    _lossq_sectioned_excel_original_repair_v1 = lossq_sectioned_excel_loss_run_repair_v1

    def _lossq_ps_clean_v1(value):
        if value is None:
            return ""
        return str(value).strip()

    def _lossq_ps_norm_v1(value):
        import re
        return re.sub(r"[^a-z0-9]+", "", _lossq_ps_clean_v1(value).lower())

    def _lossq_ps_money_v1(value):
        import re

        raw = _lossq_ps_clean_v1(value)
    ...
    def _lossq_ps_date_v1(value):
        ...
        return _lossq_ps_clean_v1(value)

    def _lossq_ps_title_v1(value):
        raw = _lossq_ps_clean_v1(value)
        ...
        for bad, good in fixes.items():
            titled = titled.replace(bad, good)

        return titled

    def _lossq_ps_find_file_path_v1(args, kwargs):
        from pathlib import Path

        candidates = list(args or []) + list((kwargs or {}).values())
        ...
        for candidate in candidates:
            ...
            if raw.lower().endswith((".xlsx", ".xlsm", ".xls")) and Path(raw).exists():
                return raw

        return ""

    def _lossq_ps_extract_summary_workbook_v1(file_path):
        import re
        from openpyxl import load_workbook
        ...
        try:
            ...
            print("LOSSQ_SECTIONED_EXCEL_POLICY_SUMMARY_RESULT_WRAPPER_V1_OPEN_ERROR:", str(exc)[:300])
            return None

        if "Policy Summary" not in wb.sheetnames or "Claims Detail" not in wb.sheetnames:
            return None

        summary_ws = wb["Policy Summary"]
        claims_ws = wb["Claims Detail"]

        summary_rows = [[cell.value for cell in row] for row in summary_ws.iter_rows()]
        claims_rows = [[cell.value for cell in row] for row in claims_ws.iter_rows()]
        ...
        first_summary_text = " ".join(_lossq_ps_clean_v1(cell) for cell in (summary_rows[0] if summary_rows else []) if _lossq_ps_clean_v1(cell))

        if first_summary_text:
            carrier_part = re.split(r"\s+[–-]\s+", first_summary_text, maxsplit=1)[0]
            carrier = _lossq_ps_title_v1(carrier_part)

        def next_value(row, index):
            if index + 1 < len(row):
                return _lossq_ps_clean_v1(row[index + 1])
            return ""

        for row in summary_rows:
            for index, cell in enumerate(row):
                label = _lossq_ps_norm_v1(cell)
                value = next_value(row, index)
                ...
                elif label in {"policy", "policy#", "policynumber", "policyno"} or label.startswith("policy#"):
                    if not re.search(r"\d{4}[-/]\d{1,2}[-/]\d{1,2}\s*(?:to|through|thru|-|–)\s*\d{4}[-/]\d{1,2}[-/]\d{1,2}", value, flags=re.I):
                        policy_number = value

                elif label in {"province", "stateprovince"}:
                    province = value

                elif label in {"currency"}:
                    currency = value

                elif label in {"ibclob", "lob", "lineofbusiness"}:
                    line_of_business = "CGL" if re.search(r"\bCGL\b", value, flags=re.I) else value

                elif label in {"reportdate", "evaluationdate", "valuationdate"}:
                    evaluation_date = _lossq_ps_date_v1(value)

                elif label in {"occurrencelimit", "policylimit", "limits"}:
                    clean_limit = value.replace("per occ", "occ").replace("per occurrence", "occ")
                    limits = f"CGL: {clean_limit}" if re.search(r"\bCGL\b", line_of_business or "", flags=re.I) else clean_limit

                elif label in {"policyperiod", "period"}:
                    period = value
                    match = re.search(
        ...
        if not policy_number:
            for row in claims_rows[:3]:
                row_text = " ".join(_lossq_ps_clean_v1(cell) for cell in row if _lossq_ps_clean_v1(cell))
                match = re.search(r"\b[A-Z]{1,6}(?:-[A-Z0-9]+){2,}\b", row_text)
        ...
        for row_index, row in enumerate(claims_rows):
            normalized = [_lossq_ps_norm_v1(cell) for cell in row]

            if any(value in {"claim", "claim#", "claimnumber", "claimno"} for value in normalized):
                header_index = row_index
                header_map = {value: index for index, value in enumerate(normalized) if value}
                break
        ...
        def col(*names):
            for name in names:
                normalized_name = _lossq_ps_norm_v1(name)
        ...
        claim_col = col("Claim #", "Claim Number", "Claim No")
        loss_date_col = col("Date of Loss", "Loss Date")
        reported_col = col("Date Reported", "Reported Date")
        claimant_col = col("Claimant")
        description_col = col("Incident Description", "Description", "Loss Description")
        coverage_col = col("Coverage Trigger", "Coverage", "Line", "Line of Business")
        status_col = col("Status")
        paid_col = col("Paid (CAD)", "Paid")
        expense_col = col("Expense (CAD)", "Expense")
        reserve_col = col("Reserve (CAD)", "Reserve")
        incurred_col = col("Incurred (CAD)", "Incurred", "Total Incurred")

        claims = []

        for row in claims_rows[header_index + 1:]:
            claim_number = _lossq_ps_clean_v1(row[claim_col]) if claim_col is not None and claim_col < len(row) else ""

            if not claim_number:
                continue

            if claim_number.upper() == "TOTAL":
                continue

            if re.fullmatch(r"\d{4}\s*[-–]\s*\d{4}", claim_number):
                continue

            paid = _lossq_ps_money_v1(row[paid_col]) if paid_col is not None and paid_col < len(row) else 0.0
            expense = _lossq_ps_money_v1(row[expense_col]) if expense_col is not None and expense_col < len(row) else 0.0
            reserve = _lossq_ps_money_v1(row[reserve_col]) if reserve_col is not None and reserve_col < len(row) else 0.0
            incurred = _lossq_ps_money_v1(row[incurred_col]) if incurred_col is not None and incurred_col < len(row) else paid + expense + reserve

            coverage = _lossq_ps_clean_v1(row[coverage_col]) if coverage_col is not None and coverage_col < len(row) else line_of_business
            status = _lossq_ps_clean_v1(row[status_col]) if status_col is not None and status_col < len(row) else ""
            ...
            claim = {
                ...
                "policyNumber": policy_number,
                "line_of_business": coverage or line_of_business,
                "line": coverage or line_of_business,
                "status": status,
                "paid": paid,
                "paid_amount": paid,
                "expense": expense,
                ...
                "incurred": incurred,
                "total_incurred": incurred,
                "loss_date": _lossq_ps_date_v1(row[loss_date_col]) if loss_date_col is not None and loss_date_col < len(row) else "",
                "date_of_loss": _lossq_ps_date_v1(row[loss_date_col]) if loss_date_col is not None and loss_date_col < len(row) else "",
                "reported_date": _lossq_ps_date_v1(row[reported_col]) if reported_col is not None and reported_col < len(row) else "",
                "date_reported": _lossq_ps_date_v1(row[reported_col]) if reported_col is not None and reported_col < len(row) else "",
                "claimant": _lossq_ps_clean_v1(row[claimant_col]) if claimant_col is not None and claimant_col < len(row) else "",
                "description": _lossq_ps_clean_v1(row[description_col]) if description_col is not None and description_col < len(row) else "",
                "loss_description": _lossq_ps_clean_v1(row[description_col]) if description_col is not None and description_col < len(row) else "",
            }

            claims.append(claim)

        if not claims or not policy_number:
            return None

        total_incurred = sum(_lossq_ps_money_v1(claim.get("total_incurred")) for claim in claims)
        open_claims = len([claim for claim in claims if _lossq_ps_clean_v1(claim.get("status")).lower() != "closed"])
        ...
        exposure_header_index = None
        exposure_header = []

        for row_index, row in enumerate(summary_rows):
            if any(_lossq_ps_norm_v1(cell) == "exposureclass" for cell in row):
                exposure_header_index = row_index
                exposure_header = [_lossq_ps_norm_v1(cell) for cell in row]
                break

        if exposure_header_index is not None:
            revenue_col = None
            square_col = None
            employee_col = None
            premium_col = None

            for index, header in enumerate(exposure_header):
                if "revenue" in header or "receipts" in header:
                    revenue_col = index
                elif "squarefootage" in header:
                    square_col = index
                elif "employee" in header:
                    employee_col = index
                elif "premium" in header:
                    premium_col = index

            for row in summary_rows[exposure_header_index + 1:]:
                label = _lossq_ps_clean_v1(row[0] if row else "")

                if label.upper() == "TOTAL":
                    if premium_col is not None and premium_col < len(row):
                        current_premium = _lossq_ps_money_v1(row[premium_col])
                    continue

                if revenue_col is not None and revenue_col < len(row):
                    revenue = max(revenue, _lossq_ps_money_v1(row[revenue_col]))

                if employee_col is not None and employee_col < len(row):
                    employee_raw = _lossq_ps_clean_v1(row[employee_col])

                    if employee_raw and employee_raw not in {"—", "-"}:
                        employee_count = employee_raw

                if square_col is not None and square_col < len(row):
                    square_raw = _lossq_ps_clean_v1(row[square_col])

                    if square_raw and square_raw.upper() not in {"N/A", "NA", "—", "-"}:
                        square_footage = square_raw

        policy_row = {
            "policy_number": policy_number,
            "policyNumber": policy_number,
            "policy_type": line_of_business or "CGL",
            "coverage": line_of_business or "CGL",
            "line_of_business": line_of_business or "CGL",
            "carrier": carrier,
            "effective_date": effective_date,
            "expiration_date": expiration_date,
            "claims": len(claims),
        ...
        return {
            ...
            "expiring_premium": current_premium,
            "revenue": revenue,
    ...
    def _lossq_ps_apply_result_repair_v1(result, extracted):
        if not isinstance(result, dict) or not isinstance(extracted, dict):
            return result

        claims = extracted.get("claims") or []
        policy_rows = extracted.get("policy_rows") or []
        policy_number = extracted.get("policy_number") or ""

        if claims:
            result["claims"] = claims
            result["parsed_claims"] = claims
            result["total_claims"] = len(claims)
            result["open_claims"] = extracted.get("open_claims")
            result["total_incurred"] = extracted.get("total_incurred")

        result["policy_number"] = policy_number
        result["policyNumber"] = policy_number
        result["carrier_name"] = extracted.get("carrier")
        result["writing_carrier"] = extracted.get("carrier")
        result["carrier"] = extracted.get("carrier")
        result["line_of_business"] = extracted.get("line_of_business")
        result["effective_date"] = extracted.get("effective_date")
        result["expiration_date"] = extracted.get("expiration_date")
        result["evaluation_date"] = extracted.get("evaluation_date")
        result["policies"] = policy_rows
        result["policy_schedule"] = policy_rows
        result["policySchedule"] = policy_rows
        ...
        for key_name in profile_keys:
            profile = result.get(key_name)

            if not isinstance(profile, dict):
                continue

            profile["policy_number"] = policy_number
            profile["policyNumber"] = policy_number
            profile["main_policy"] = policy_number
            profile["carrier_name"] = extracted.get("carrier")
            profile["writing_carrier"] = extracted.get("carrier")
            profile["carrier"] = extracted.get("carrier")
            profile["business_name"] = profile.get("business_name") or extracted.get("insured")
            profile["insured_name"] = profile.get("insured_name") or extracted.get("insured")
            profile["line_of_business"] = extracted.get("line_of_business")
            profile["effective_date"] = extracted.get("effective_date")
            profile["expiration_date"] = extracted.get("expiration_date")
            profile["evaluation_date"] = extracted.get("evaluation_date")
            profile["current_premium"] = extracted.get("current_premium")
            profile["expiring_premium"] = extracted.get("expiring_premium")
            profile["revenue"] = extracted.get("revenue")
            profile["revenue_sales"] = extracted.get("revenue")
            profile["employee_count"] = extracted.get("employee_count")
            profile["square_footage"] = extracted.get("square_footage")
            profile["coverage_limit"] = extracted.get("limits")
            profile["limits"] = extracted.get("limits")
            profile["policies"] = policy_rows
            profile["policy_schedule"] = policy_rows
            profile["policySchedule"] = policy_rows

        print("LOSSQ_SECTIONED_EXCEL_POLICY_SUMMARY_RESULT_WRAPPER_V1:", {
            "policy_number": policy_number,
            "claims": len(claims),
            "total_incurred": extracted.get("total_incurred"),
        })

        return result

    def lossq_sectioned_excel_loss_run_repair_v1(*args, **kwargs):
        result = _lossq_sectioned_excel_original_repair_v1(*args, **kwargs)

        try:
            file_path = _lossq_ps_find_file_path_v1(args, kwargs)

            if not file_path:
                return result

            extracted = _lossq_ps_extract_summary_workbook_v1(file_path)

            if not extracted:
                return result

            return _lossq_ps_apply_result_repair_v1(result, extracted)

        except Exception as wrapper_exc:
            print("LOSSQ_SECTIONED_EXCEL_POLICY_SUMMARY_RESULT_WRAPPER_V1_ERROR:", str(wrapper_exc)[:300])
            return result
