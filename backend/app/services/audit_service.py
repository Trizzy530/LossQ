...
def write_audit_event(db: Session, current_user: Any, action: str, resource_type: str = "", resource_id: str = "", details: Any = None, request: Any = None) -> AuditLog | None:
    try:
        details_text = None if details is None else (details if isinstance(details, str) else json.dumps(details, default=str))
        event = AuditLog(
